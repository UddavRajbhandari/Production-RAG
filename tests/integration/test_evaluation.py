...
class TestEvaluationMetrics:
    ...
    def test_compute_answer_completeness_medium(self) -> None:
        """Test completeness for medium answer."""
        answer = "This is a medium length answer that provides some details."
        completeness = compute_answer_completeness(answer)
        assert completeness == 0.6

    def test_compute_answer_completeness_long(self) -> None:
        """Test completeness for long answer."""
        answer = (
            "This is a very long answer that provides extensive details "
            "about the topic and covers all the important aspects."
        )
        completeness = compute_answer_completeness(answer)
        assert completeness == 0.8

    def test_compute_answer_completeness_very_long(self) -> None:
        """Test completeness for very long answer."""
        answer = "A" * 150  # More than 100 words
        completeness = compute_answer_completeness(answer)
